	...
	function pageSearch(num) {
		if (num == '1') {
			...
			document.getElementById("number").value = document
					.getElementById("prevPage").value;
		} else if (num == 'n') {
			document.getElementById("number").value = document
					.getElementById("nextPage").value;
		} else {
			document.getElementById("number").value = document
...
<body onload='init()'>
			...
			<form method="post" action="search.action" id='sform'>
				...
				<dt>
					<ul>
						...
						<li>标题：<input type="text" class="text"
							value='<s:property value="title"/>' name="title" /> <label
							class="ui-blue"><input type="button" onclick='doSearch()'
								name="search" value="搜索房屋" /> </label>
						</li>
					</ul>
				</dt>
				<dd>
				...
				<dd>
					<ul>
						...
						<li><select name='type_id' id='housetype'>
								<option value=''>不限</option>
	...
	<div class="main wrap">
			...
			<s:if test="#result!=null">
				<s:iterator value="#result" status="status">
					<tr>
						...
						<td class="house-price"><span><s:property
									value="price" /> </span>元/月</td>
					</tr>
				</s:iterator>
			</s:if>
		</table>
		<div class="pager">
			...
			</s:url> --%> <s:a href="javascript:pageSearch('1')">首页</s:a> <!-- <a href="/HouseRent/index.action?number=1">首页</a> -->
				</li>
				<li>
					<%-- 			<s:url id="pre" value="search.action">
				<s:param name="number" value="prev"></s:param>
			</s:url> --%> <s:a href="javascript:pageSearch('p')">上一页</s:a> <%-- <a href='/HouseRent/index.action?number=<s:property value="prev"/>'>上一页</a> --%>
				</li>
				<li>
					<%-- 			<s:url id="nex" value="search.action">
				<s:param name="number" value="next"></s:param>
			</s:url> --%> <s:a href="javascript:pageSearch('n')">下一页</s:a> <%-- <a href='/HouseRent/index.action?number=<s:property value="next"/>'>下一页</a> --%>
				</li>
				<li>
					<%-- 			<s:url id="last" value="search.action">
				<s:param name="number" value="total"></s:param>
			</s:url> --%> <s:a href="javascript:pageSearch('t')">末页</s:a> <%-- <a href='/HouseRent/index.action?number=<s:property value="total"/>'>末页</a> --%>
				</li>
			</ul>
			<span class="total"><s:property value="curPage" />/<s:property
					value="total" />页</span>
		</div>
		<s:debug></s:debug>
	</div>
